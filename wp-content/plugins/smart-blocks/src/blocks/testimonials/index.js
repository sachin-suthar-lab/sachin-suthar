...
const TEMPLATE = [
	[ 'smart-blocks/testimonial', { quote: 'Sachin rebuilt our marketing site as an FSE theme and a small custom block library — the editor experience went from "open a ticket" to "ship it yourself" overnight.', name: 'Priya Menon',    role: 'Head of Marketing · B2B SaaS' } ],
	...
	[ 'smart-blocks/testimonial', { quote: 'The Gutenberg block library Sachin built is now reused across 30+ client sites. Real engineering, not just markup.',                                                      name: 'Sofia Alvarez',  role: 'Tech Lead · Digital Agency' } ],
	[ 'smart-blocks/testimonial', { quote: 'Migrated 14 country sites into one multisite network with zero downtime and a faster editorial workflow on the other side. Quiet, calm, and exact.',                     name: 'Jonas Berg',     role: 'Director of Web · Enterprise' } ],
];
...
registerBlockType( metadata.name, {
	...
	save( { attributes } ) {
		...
		const innerProps = useInnerBlocksProps.save( { className: 'sb-testimonials__grid' } );
		return (
			<section { ...blockProps }>
