...
registerBlockType( metadata.name, {
	edit( { attributes, setAttributes } ) {
		...
		const blockProps = useBlockProps( { className: 'sb-section sb-cta-wrap' } );
		return (
			<>
				<InspectorControls>
	...
	save( { attributes } ) {
		const { heading, dek, primaryText, primaryUrl, secondaryText, secondaryUrl } = attributes;
		const blockProps = useBlockProps.save( { className: 'sb-section sb-cta-wrap sb-reveal' } );
		return (
			<section { ...blockProps }>
				<div className="sb-cta">
					<RichText.Content tagName="h2" value={ heading } />
					<RichText.Content tagName="p"  value={ dek } />
					<div className="sb-cta__buttons">
